...
backend/
  app/
    ...
    notify.py          escalation delivery — SMTP, DRY RUN unless configured
    rulebook_edit.py   the surgical YAML edit behind POST /api/rulebook
    derive_all.py      build step: cases, rule_hits, audit_log, rollups
    alerts_run.py      build step: one alert per HIGH case, routed by scope
    seed_users.py      the four demo accounts — a build step, run last
    engine/
      derive.py        F1  both ladders + the derived feature dictionary
                       (DOMAIN-MODEL.md (b), (c), (f))
      rulebook.py      F2  load() + evaluate()
      score.py         F3+F4+F5 composite score, the corroboration bonus,
                       trace, coverage_pct
      memo.py          plain-language memo — template f-string, NOT an LLM
      audit.py         F6  append-only log + recompute against the snapshot
    ml/                F7  duplicates, anomaly badge, delay forecast, graph
    ablation/          F9  fields, measure, rank, report, run
    routers/
...
docs/data/             DATA-PROFILE.md, INGEST-EXPECTATIONS.md
docs/domain/           DOMAIN-MODEL.md
docs/contract/         case_detail.json (frozen), fixtures.md
docs/design/           REDESIGN-SPEC.md
docs/api/              ROLE-SCOPING-PLAN.md
docs/reports/          DATA-GAP-RECOMMENDATION.md (regenerated by ablation.run)
docs/context/          REPO-CONTEXT.md — a dated snapshot of the INHERITED
                       LEAKPROOF repo, kept as provenance. It describes what
